...
struct rule *rule_find (const char *name) {
	...
	if (p) {
		...
		p++;
		if (dot) {
                    memmove(p, dot, strlen(dot) + 1);
		} else {
                    *p = '\0';
		}
	}
        key = hashtab_get_key (&hashtab_rules, n);
	free(n);
	if (!key) {
            return 0;
        }
    ...
    while (*old_body) {
	    ...
	    if (l + dst_len + 1 >= size - pos) {
	        size += l + dst_len + 1 + strlen(old_body);
                new_body = realloc(new_body, size);
	    }
...
static int pipe_command (const char *filename, unsigned long line_no, char *input, int is_ignoring_errors, const char *name) {
    ...
    */
    ...
    if (!internal) {
        strcat(p, "/C ");
        strcat(p, lpApplicationName);
        strcat(p, " ");
    }
    strcat(p, lpCommandLine);
    free(lpCommandLine);
    lpCommandLine = p;
    ...
    
    {
        ...
        while (ReadFile (hStdOutPipeRead, p, end - p, &dwRead, 0) && dwRead > 0) {
...
static int pipe_command (const char *filename, unsigned long line_no, char *input, int is_ignoring_errors, const char *name) {
    ...
    } else if (lbuf.start[0] && print_output) {
        printf ("%s\n", lbuf.start);
    }
...
static int rule_use (struct rule *r, char *name) {
    ...
    char *p, *star_name, *lesser_name, *n, *fname, *hash_name, *dot;
    int ret;
    ...
    for (dep = r->deps; dep; dep = dep->next) {
        ...
        p = strchr(dep->name, '%');
	if (p) {
		n = xmalloc(strlen(dep->name) + strlen(name));
		n[0] = 0;
		strncat(n, dep->name, p - dep->name);
		fname = strrchr(name, '/');
		if (!fname) {
			fname = strrchr(name, '\\');
		}
		...
		dot = strrchr(fname, '.');
		if (dot) {
			strncat(n, fname, dot - fname);
		} else {
			strcat(n, fname);
		}
		strcat(n, p + 1);
        	ret = rule_search_and_build (n);
                cstr_cat (&str, n, strlen (n));
		free(n);
		if (ret) {
			return ret;
		}
    ...
    hash_name = xstrdup(fname);
    dot = strrchr(hash_name, '.');
    if (dot) {
	    *dot = '\0';
    }
