...
if (!isset($_SESSION['UserData']["user"])) {
	header('HTTP/1.1 407 No session');
	exit(-1);
}



$dn = get_user_home(get_user()) . "/uploads/";
...
function has_permission($dest, $user, $type)
{
	...
	return has_permission(substr(0, strrpos($dest, "/")));
}

function has_write_permission($dest)
{
	global $user, $dn;
	$dr = $dest;
	$l = strlen($dn);
	if (0 === strncmp($dn, $dr, $l) && $dr[$l] == '/') {
		return true; // we are in user's home directory
	}
