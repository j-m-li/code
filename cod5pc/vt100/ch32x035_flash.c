/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_flash.c Author
 *: WCH Version            : V1.0.1 Date               : 2024/12/11 Description
 ...
 *
 * @return  FLASH Status - The returned value can be: FLASH_BUSY,
 * FLASH_ERROR_PG, FLASH_ERROR_WRP, FLASH_COMPLETE , FLASH_TIMEOUT or FLASH_RDP.
 */
FLASH_Status FLASH_EnableWriteProtection(uint32_t FLASH_Pages) {
	uint8_t WRP0_Data = 0xFF, WRP1_Data = 0xFF, WRP2_Data = 0xFF,
		WRP3_Data = 0xFF;
	uint32_t buf[4];
	uint8_t i;
	FLASH_Status status = FLASH_COMPLETE;
	...
	if ((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET) {
		...
		if (status == FLASH_COMPLETE) {
			...
			buf[2] = ((
			    uint32_t)(((uint32_t)(WRP0_Data) & 0x00FF) +
				      (((uint32_t)(~WRP0_Data) & 0x00FF) << 8) +
				      (((uint32_t)(WRP1_Data) & 0x00FF) << 16) +
				      (((uint32_t)(~WRP1_Data) & 0x00FF)
				       << 24)));
			buf[3] = ((
			    uint32_t)(((uint32_t)(WRP2_Data) & 0x00FF) +
				      (((uint32_t)(~WRP2_Data) & 0x00FF) << 8) +
				      (((uint32_t)(WRP3_Data) & 0x00FF) << 16) +
				      (((uint32_t)(~WRP3_Data) & 0x00FF)
				       << 24)));
...
FLASH_Status FLASH_UserOptionByteConfig(uint8_t OB_IWDG, uint8_t OB_STOP,
	...
	if ((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET) {
		...
		UserByte =
		    ...
		    (uint8_t)(OB_STOP |
			      (uint8_t)(OB_STDBY |
					(uint8_t)(OB_RST | (uint8_t)0xE0)));

		for (i = 0; i < 4; i++) {
			buf[i] = *(uint32_t *)(OB_BASE + 4 * i);
		}
		buf[0] =
		    ((uint32_t)((((uint32_t)(UserByte) & 0x00FF) << 16) +
				(((uint32_t)(~UserByte) & 0x00FF) << 24))) +
		    0x00005AA5;
 ...
 * @return  none
 */
void FLASH_Lock_Fast(void) { FLASH->CTLR |= CR_FLOCK_Set; }
...
FLASH_Status FLASH_ROM_ERASE(uint32_t StartAddr, uint32_t Length) {
	...
	if (Length0 >= Size_32KB) // front
	{
		...
		ROM_ERASE((Addr0 + Length0), ((Length - Length0) >> 15),
			  Size_32KB);
	}
	...
	if (Length0 >= Size_1KB) // front
	{
		...
		ROM_ERASE((Addr0 + Length0), ((Length - Length0) >> 10),
			  Size_1KB);
	}
