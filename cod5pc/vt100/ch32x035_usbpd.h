/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_usbpd.h Author
 *: WCH Version            : V1.0.0 Date               : 2023/04/06 Description
 *: This file contains all the functions prototypes for the USBPD firmware
 *library.
 *********************************************************************************
...
#define TX_BIT_BACK                                                            \
	(1 << 6) /* Indicates the current bit status of the BMC when sending   \
		    the code */
#define BMC_BYTE_HI                                                            \
	(1 << 7) /* Indicates the current half-byte status of the PD data      \
 ...
 * 1-High threshold input (2.2V typical), to reduce the I/O power consumption
 * during PD communication 0-Normal GPIO threshold input
 * *******************************************************/
#define USBPD_PHY_V33 (1 << 8)
/**********************************************************
 * PD transceiver PHY pull-up limit configuration bits:
 * 1-Direct use of VDD for GPIO applications or PD applications with VDD voltage
 * of 3.3V 0-LDO buck enabled, limited to approx 3.3V, for PD applications with
 * VDD more than 4V
 * ********************************************************/
...
#define UPD_TMR_TX_48M                                                         \
	(80 - 1) /* timer value for USB PD BMC transmittal @Fsys=48MHz */
#define UPD_TMR_RX_48M                                                         \
	(120 - 1) /* timer value for USB PD BMC receiving @Fsys=48MHz */
#define UPD_TMR_TX_24M                                                         \
	(40 - 1) /* timer value for USB PD BMC transmittal @Fsys=24MHz */
#define UPD_TMR_RX_24M                                                         \
	(60 - 1) /* timer value for USB PD BMC receiving @Fsys=24MHz */
#define UPD_TMR_TX_12M                                                         \
	(20 - 1) /* timer value for USB PD BMC transmittal @Fsys=12MHz */
#define UPD_TMR_RX_12M                                                         \
	(30 - 1) /* timer value for USB PD BMC receiving @Fsys=12MHz */
...
#define UPD_SOP0                                                               \
	(TX_SEL1_SYNC1 | TX_SEL2_SYNC1 | TX_SEL3_SYNC1 |                       \
	 TX_SEL4_SYNC2) /* SOP1 */
#define UPD_SOP1                                                               \
	(TX_SEL1_SYNC1 | TX_SEL2_SYNC1 | TX_SEL3_SYNC3 |                       \
	 TX_SEL4_SYNC3) /* SOP2 */
#define UPD_SOP2                                                               \
	(TX_SEL1_SYNC1 | TX_SEL2_SYNC3 | TX_SEL3_SYNC1 |                       \
	 TX_SEL4_SYNC3) /* SOP3 */
#define UPD_HARD_RESET                                                         \
	(TX_SEL1_RST1 | TX_SEL2_RST1 | TX_SEL3_RST1 |                          \
	 TX_SEL4_RST2) /* Hard Reset*/
#define UPD_CABLE_RESET                                                        \
	(TX_SEL1_RST1 | TX_SEL2_SYNC1 | TX_SEL3_RST1 |                         \
	 TX_SEL4_SYNC3) /* Cable Reset*/
...
typedef enum {
	...
	STA_RX_DR_SWAP_ACCEPT,	/* 20: Waiting to receive the answer ACCEPT from
				   DR_SWAP */
	STA_TX_PR_SWAP,		/* 21: Send PR_SWAP */
	STA_RX_PR_SWAP_ACCEPT,	/* 22: Waiting to receive the answer ACCEPT from
				   PR_SWAP */
	STA_RX_PR_SWAP_PS_RDY,	/* 23: Waiting to receive the answer PS_RDY from
				   PR_SWAP */
	...
	STA_RX_APD_PS_RDY_WAIT, /* 28: Waiting for PS_RDY from the receiving
				   adapter */
...
typedef union {
	struct _BITS_ {
		UINT8 Msg_Recvd : 1; /* Notify the main program of the receipt
					of a PD packet */
		UINT8 Connected : 1; /* PD Physical Layer Connected Flag */
		UINT8 Stop_Det_Chk : 1;	   /* 0-Enable detection; 1-Disable
					      disconnection detection */
		UINT8 PD_Role : 1;	   /* 0-UFP; 1-DFP */
		UINT8 PR_Role : 1;	   /* 0-Sink; 1-Source */
		UINT8 Auto_Ack_PRRole : 1; /* Role used by auto-responder
					      0:SINK; 1:SOURCE */
		...
		UINT8 HPD_Det_Chk : 1;	 /* 0-turn off HPD connection detection;
					    1-turn on HPD connection detection */
		UINT8 CC_Sel_En : 1; /* 0-CC channel selection toggle enable;
					1-CC channel selection toggle disable */
		UINT8
		CC_Sel_State : 1;	/* 0-CC channel selection switches to 0;
					   1-CC channel selection switches to 1 */
		UINT8 PD_Comm_Succ : 1; /* 0-PD communication unsuccessful; 1-PD
...
typedef struct _PD_CONTROL {
	...
	CC_STATUS
	PD_State_Last;	      /* PD communication status machine (last value) */
	...
	UINT8 Det_Sel_Cnt;    /* Number of SEL toggles for PD connection status
				 detection */
	...
	UINT8 Mode_Try_Cnt; /* Number of retries for current mode, highest bit
			       marks mode */
