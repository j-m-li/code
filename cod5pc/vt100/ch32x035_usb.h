 ...
 *WCH Version            : V1.0.0 Date               : 2024/08/02 Description :
 *This file contains all the functions prototypes for the USB firmware library.
 *********************************************************************************
...
#define CDC_SET_LINE_CODING                                                    \
	0x20 /* Configures DTE rate, stop-bits, parity, and                    \
		number-of-character */
#define CDC_SET_LINE_CTLSTE                                                    \
	0X22 /* This request generates RS-232/V.24 style control signals */
#define CDC_SEND_BREAK                                                         \
	0X23 /* Sends special carrier modulation used to specify RS-232 style  \
		break */
#endif
/*******************************************************************************/
...
#define USBFS_U_IS_NAK                                                         \
	0x80 // RO, indicate current USB transfer is NAK received for USB device
	     // mode
...
#define USBFS_UIF_FIFO_OV                                                      \
	0x10 // FIFO overflow interrupt flag for USB, direct bit address clear
	     // or write 1 to clear
#define USBFS_UIF_HST_SOF                                                      \
	0x08 // host SOF timer interrupt flag for USB host, direct bit address
	     // clear or write 1 to clear
#define USBFS_UIF_SUSPEND                                                      \
	0x04 // USB suspend or resume event interrupt flag, direct bit address
	     // clear or write 1 to clear
#define USBFS_UIF_TRANSFER                                                     \
	0x02 // USB transfer completion interrupt flag, direct bit address clear
	     // or write 1 to clear
#define USBFS_UIF_DETECT                                                       \
	0x01 // device detected event interrupt flag for USB host mode, direct
	     // bit address clear or write 1 to clear
#define USBFS_UIF_BUS_RST                                                      \
	0x01 // bus reset event interrupt flag for USB device mode, direct bit
...
#define USBFS_UIS_TOKEN_MASK                                                   \
	0x30 // RO, bit mask of current token PID code received for USB device
	     // mode
...
#define USBFS_UIS_ENDP_MASK                                                    \
	0x0F // RO, bit mask of current transfer endpoint number for USB device
	     // mode
#define USBFS_UIS_H_RES_MASK                                                   \
	0x0F // RO, bit mask of current transfer handshake response for USB host
	     // mode: 0000=no response, time out from device, others=handshake
	     // response PID received

/* R8_UDEV_CTRL */
#define USBFS_UD_PD_DIS                                                        \
	0x80 // disable USB UDP/UDM pulldown resistance: 0=enable pulldown,
	     // 1=disable
...
#define USBFS_UEP_T_AUTO_TOG                                                   \
	(1 << 4) // enable automatic toggle after successful transfer completion
		 // on endpoint 1/2/3: 0=manual toggle, 1=automatic toggle
#define USBFS_UEP_T_TOG                                                        \
	(1 << 6) // prepared data toggle flag of USB endpoint X transmittal
		 // (IN): 0=DATA0, 1=DATA1
#define USBFS_UEP_T_RES_MASK                                                   \
	(3 << 0) // bit mask of handshake response type for USB endpoint X
		 // transmittal (IN)
...
#define USBFS_UEP_R_RES_MASK                                                   \
	(3 << 2) // bit mask of handshake response type for USB endpoint X
		 // receiving (OUT)
...
#define USBFS_UH_PORT_EN                                                       \
	0x01 // enable USB port: 0=disable, 1=enable port, automatic disabled if
	     // USB device detached
...
#define USBFS_UH_R_AUTO_TOG                                                    \
	0x10 // enable automatic toggle after successful transfer completion:
	     // 0=manual toggle, 1=automatic toggle
#define USBFS_UH_R_TOG                                                         \
	(1 << 7) // expected data toggle flag of host receiving (IN): 0=DATA0,
		 // 1=DATA1
...
#define USBFS_UH_T_RES                                                         \
	0x01 // expected handshake response type for host transmittal
	     // (SETUP/OUT): 0=ACK (ready), 1=no response, time out from device,
