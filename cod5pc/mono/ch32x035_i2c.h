/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_i2c.h Author :
...
typedef struct {
	uint32_t I2C_ClockSpeed; /* Specifies the clock frequency.
				    This parameter must be set to a value lower
				    than 400kHz */
	...
	uint16_t I2C_AcknowledgedAddress; /* Specifies if 7-bit or 10-bit
					     address is acknowledged. This
 ...
 *
 * After master use I2C_GenerateSTART() function sending the START condition,the
 * master has to wait for event 5(the Start condition has been correctly
 ...
 * master use I2C_Send7bitAddress() function sends the address of the slave(s)
 * with which it will communicate it also determines master as transmitter or
 * Receiver. Then the master has to wait that a slave acknowledges his address.
 * If an acknowledge is sent on the bus, one of the following events will be
 * set:
 *
 *
 *
 ...
 * been acknowledged. then the master has to check one of the following events
 ...
 * data then to wait on event EVT8 or EVT8_2. These two events are similar:
 ...
 *     Using EVT8_2  will leads to a slower communication  speed but will more
 * reliable . EVT8_2 is also more suitable than EVT8 for testing on the last
 * data transmission
 *
 *
 *  Note:
 *  In case the  user software does not guarantee that this event EVT7 is
 * managed before the current byte end of transfer, then user may check on
 * I2C_EVENT_MASTER_BYTE_RECEIVED and I2C_FLAG_BTF flag at the same time .But in
 * this case the communication may be slower.
 *
 *
 */
 ...
 * the bus. If the acknowledge feature is enabled through function
 * I2C_AcknowledgeConfig()),The peripheral generates an ACK condition on the
 * bus.
 *
 *
 *
 ...
 * transmission shall end . The slave device has to stop sending data bytes and
 * wait a Stop condition from bus.
 *
 *  Note:
 *  If the  user software does not guarantee that the event 2 is
 *  managed before the current byte end of transfer, User may check on
 * I2C_EVENT_SLAVE_BYTE_RECEIVED and I2C_FLAG_BTF flag at the same time . In
 * this case the communication will be slower.
 *
 */
 ...
 *    It compares the status registers (STARR1 and STAR2) content to a given
 *event (can be the combination of more flags). If the current status registers
 *includes the given flags  will return SUCCESS. and  if the current status
 *registers miss flags will returns ERROR.
 *    - When to use:
 ...
 *        the I2C_CheckEvent() function may return SUCCESS despite the
 *communication in corrupted state.  it is suggeted to use error interrupts to
 *monitor the error events and handle them in IRQ handler.
 *
 *
 *        Note:
 *        The following functions are recommended for error management: :
 *          - I2C_ITConfig() main function of configure and enable the error
 *interrupts.
 *          - I2Cx_ER_IRQHandler() will be called when the error interrupt
 *happen. Where x is the peripheral instance (I2C1, I2C2 ...)
 *          -  I2Cx_ER_IRQHandler() will call I2C_GetFlagStatus() or
 *I2C_GetITStatus() functions to determine which error occurred.
 *          - I2C_ClearFlag() \ I2C_ClearITPendingBit() \ I2C_SoftwareResetCmd()
 *            \ I2C_GenerateStop() will be use to clear the error flag and
 *source, and return to correct communication status.
 *
 *
 *  b) Second way:
 *     Using the function to get a single word(uint32_t) composed of status
 *register 1 and register 2. (Status Register 2 value is shifted left by 16 bits
 *and concatenated to Status Register 1).
 *     - When to use:
 *
 ...
 *       - This function can be used to monitor the status of multiple flags
 *simultaneously.
 *       - Contrary to the I2C_CheckEvent () function, this function can choose
 *the time to accept the event according to the user's needs (when all event
 *flags are set and no other flags are set, or only when the required flags are
 *set)
 *
 *     - Limitations:
 ...
 *
 *     - Limitations:
 *        - Call this function to access the status register. Some flag bits may
 *be cleared.
