/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_tim.h Author :
...
typedef struct {
	uint16_t TIM_Prescaler; /* Specifies the prescaler value used to divide
				   the TIM clock. This parameter can be a number
				   between 0x0000 and 0xFFFF */

	uint16_t TIM_CounterMode; /* Specifies the counter mode.
				     This parameter can be a value of @ref
				     TIM_Counter_Mode (TIM1 and TIM2) */
	...
	uint8_t
	    TIM_RepetitionCounter; /* Specifies the repetition counter value.
				      Each time the RCR downcounter reaches
				      zero, an update event is generated and
				      counting restarts from the RCR value (N).
				      This means in PWM mode that (N+1)
				      corresponds to:
					 - the number of PWM periods in
				      edge-aligned mode
					 - the number of half PWM period in
				      center-aligned mode This parameter must be
...
typedef struct {
	...
	uint16_t TIM_OutputNState; /* Specifies the TIM complementary Output
				      Compare state. This parameter can be a
				      value of @ref TIM_Output_Compare_N_state
				      @note This parameter is valid only for
				      TIM1 and TIM8. */
	...
	uint16_t TIM_OCNPolarity; /* Specifies the complementary output
	...
	uint16_t
	    TIM_OCNIdleState; /* Specifies the TIM Output Compare pin state
				 during Idle state. This parameter can be a
...
typedef struct {
	...
	uint16_t TIM_ICPolarity; /* Specifies the active edge of the input
				    signal. This parameter can be a value of
...
typedef struct {
	uint16_t
	    TIM_OSSRState; /* Specifies the Off-State selection used in Run
			      mode. This parameter can be a value of @ref
			      OSSR_Off_State_Selection_for_Run_mode_state */

	uint16_t
	    TIM_OSSIState; /* Specifies the Off-State used in Idle state.
			      This parameter can be a value of @ref
	...
	uint16_t TIM_DeadTime; /* Specifies the delay time between the
				  switching-off and the switching-on of the
				  outputs. This parameter can be a number
				  between 0x00 and 0xFF  */

	uint16_t TIM_Break; /* Specifies whether the TIM Break input is enabled
			       or not. This parameter can be a value of @ref
			       Break_Input_enable_disable */

	uint16_t TIM_BreakPolarity; /* Specifies the TIM Break Input pin
				       polarity. This parameter can be a value
				       of @ref Break_Polarity */

	uint16_t TIM_AutomaticOutput; /* Specifies whether the TIM Automatic
					 Output feature is enabled or not. This
...
#define TIM_ICSelection_TRC                                                    \
	((uint16_t)0x0003) /* TIM Input 1, 2, 3 or 4 is selected to be         \
			      connected to TRC. */

/* TIM_Input_Capture_Prescaler */
#define TIM_ICPSC_DIV1                                                         \
	((uint16_t)0x0000) /* Capture performed each time an edge is detected  \
...
#define TIM_UpdateSource_Global                                                \
	((uint16_t)0x0000) /* Source of update is the counter                  \
			      overflow/underflow or the setting of UG bit, or  \
			      an update generation through the slave mode      \
			      controller. */
...
void TIM_OC12_SupersedeModeCmd(TIM_TypeDef *TIMx,
			       uint16_t TIM_Supersede_Mode_OC1,
			       uint16_t TIM_Supersede_Mode_OC2,
			       FunctionalState NewState);
void TIM_OC34_SupersedeModeCmd(TIM_TypeDef *TIMx,
			       uint16_t TIM_Supersede_Mode_OC3,
			       uint16_t TIM_Supersede_Mode_OC4,
			       FunctionalState NewState);
