/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_spi.c Author :
 *WCH Version            : V1.0.0 Date               : 2024/06/05 Description :
 ...
 *          When using SPI slave mode to send data, the CPOL bit should be set
 * to 1.
 *
 * @param   SPIx - where x can be 1 to select the SPI peripheral.
 ...
 *          operation to SPI_DATAR register (SPI_I2S_ReceiveData()) followed by
 * a read operation to SPI_STATR register (SPI_I2S_GetFlagStatus()).
 *          - UDR (UnderRun error) flag is cleared by a read operation to
 *          SPI_STATR register (SPI_I2S_GetFlagStatus()).
 *          - MODF (Mode Fault) flag is cleared by software sequence: a
 * read/write operation to SPI_STATR register (SPI_I2S_GetFlagStatus()) followed
 * by a write operation to SPI_CTLR1 register (SPI_Cmd() to enable the SPI).
 * @return  FlagStatus: SET or RESET.
 */
 ...
 *         sequence: a read operation to SPI_DATAR register
 * (SPI_I2S_ReceiveData()) followed by a read operation to SPI_STATR register
 * (SPI_I2S_GetITStatus()).
 *         - UDR (UnderRun Error) interrupt pending bit is cleared by a read
 *         operation to SPI_STATR register (SPI_I2S_GetITStatus()).
 *         - MODF (Mode Fault) interrupt pending bit is cleared by software
 * sequence: a read/write operation to SPI_STATR register
 * (SPI_I2S_GetITStatus()) followed by a write operation to SPI_CTLR1 register
 * (SPI_Cmd() to enable the SPI).
 * @return  none
 */
