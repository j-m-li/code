/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_opa.c Author :
 ...
 * @return         None
 */
void OPA_Lock(void) { OPA->CTLR1 |= (1 << 31); }
 ...
 * @return         None
 */
void OPA_POLL_Lock(void) { OPA->CFGR1 |= (1 << 7); }
...
void OPA_Init(OPA_InitTypeDef *OPA_InitStruct) {
	...
	if (OPA_InitStruct->OPA_NUM == OPA1) {
		...
		tmp2 |= (OPA_InitStruct->Mode << 1) |
			(OPA_InitStruct->PSEL << 3) |
		...
		tmp2 |= (OPA_InitStruct->Mode << 17) |
			(OPA_InitStruct->PSEL << 19) |
			(OPA_InitStruct->FB << 21) |
			(OPA_InitStruct->NSEL << 22);
	}

	tmp0 |= (OPA_InitStruct->PSEL_POLL) | (OPA_InitStruct->BKIN_EN << 2) |
		(OPA_InitStruct->RST_EN << 4) |
		(OPA_InitStruct->BKIN_SEL << 6) |
...
void OPA_CMP_Init(CMP_InitTypeDef *CMP_InitStruct) {
	...
	if (CMP_InitStruct->CMP_NUM == CMP1) {
		tmp1 &= 0xFFFFFFE1;
		tmp1 |=
		    (CMP_InitStruct->Mode << 1) | (CMP_InitStruct->NSEL << 2) |
		    (CMP_InitStruct->PSEL << 3) | (CMP_InitStruct->HYEN << 4);
	} else if (CMP_InitStruct->CMP_NUM == CMP2) {
		tmp1 &= 0xFFFFFC3F;
		tmp1 |=
		    (CMP_InitStruct->Mode << 6) | (CMP_InitStruct->NSEL << 7) |
		    (CMP_InitStruct->PSEL << 8) | (CMP_InitStruct->HYEN << 9);
	} else if (CMP_InitStruct->CMP_NUM == CMP3) {
		tmp1 &= 0xFFFF87FF;
		tmp1 |= (CMP_InitStruct->Mode << 11) |
			(CMP_InitStruct->NSEL << 12) |
			(CMP_InitStruct->PSEL << 13) |
			(CMP_InitStruct->HYEN << 14);
	}
 ...
 * @return  none
 */
void OPA_ClearFlag(uint16_t OPA_FLAG) { OPA->CFGR1 &= (uint16_t)~OPA_FLAG; }
