/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32x035_usart.c Author
...
#define CTLR2_LBDL_Mask                                                        \
	((uint16_t)0xFFDF) /* USART LIN Break detection Mask                   \
			    */
#define CTLR2_STOP_CLEAR_Mask                                                  \
 ...
 * disabled. USART_IT_LBD - LIN Break detection interrupt. USART_IT_TXE -
 * Transmit Data Register empty interrupt. USART_IT_TC - Transmission complete
 * interrupt. USART_IT_RXNE - Receive Data register not empty interrupt.
 ...
 * (OverRun error) and IDLE (Idle line detected) flags are cleared by software
 *            sequence: a read operation to USART_STATR register
 * (USART_GetFlagStatus()) followed by a read operation to USART_DATAR register
 * (USART_ReceiveData()).
 *            - RXNE flag can be also cleared by a read to the USART_DATAR
 * register (USART_ReceiveData()).
 *            - TC flag can be also cleared by software sequence: a read
 * operation to USART_STATR register (USART_GetFlagStatus()) followed by a write
 ...
 *         Note-
 *            - PE (Parity error), FE (Framing error), NE (Noise error), ORE
 * (OverRun error) and IDLE (Idle line detected) pending bits are cleared by
 *            software sequence: a read operation to USART_STATR register
 *            (USART_GetITStatus()) followed by a read operation to USART_DATAR
 * register (USART_ReceiveData()).
 *            - RXNE pending bit can be also cleared by a read to the
 * USART_DATAR register (USART_ReceiveData()).
 *            - TC pending bit can be also cleared by software sequence: a read
 *            operation to USART_STATR register (USART_GetITStatus()) followed
 * by a write operation to USART_DATAR register (USART_SendData()).
 *            - TXE pending bit is cleared only by a write to the USART_DATAR
